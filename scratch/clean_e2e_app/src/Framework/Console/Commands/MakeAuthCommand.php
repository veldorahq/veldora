...
CSS;
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln('');
        $output->writeln('<info>  ▲ Veldora Auth Scaffold</info>');
        $output->writeln('<comment>  Generating complete authentication layer...</comment>');
        $output->writeln('');

        $this->createMigration($basePath, $output);
        $this->createModel($basePath, $output);
        $this->createControllers($basePath, $output);
        $this->createViews($basePath, $output);
        $this->appendRoutes($basePath, $output);

        $output->writeln('');
        $output->writeln('<info>  ✔ Authentication scaffolding complete!</info>');
        $output->writeln('  Run <comment>php veldora migrate</comment> to create the users table.');
        $output->writeln('');
    ...
    private function createMigration(string $basePath, OutputInterface $output): void
    {
...
PHP;
        ...
        $output->writeln("  <comment>+</comment> database/migrations/" . basename($file));
    }

    // ── Step 2: User Model ──────────────────────────────────────────────────

    private function createModel(string $basePath, OutputInterface $output): void
    {
...
PHP;
        ...
        file_put_contents($file, $content);
        $output->writeln("  <comment>+</comment> app/Models/User.php");
    }

    // ── Step 3: Controllers ─────────────────────────────────────────────────

    private function createControllers(string $basePath, OutputInterface $output): void
    {
        ...
        foreach ($controllers as $filename => $content) {
            ...
            $output->writeln("  <comment>+</comment> app/Controllers/{$filename}");
        }
    }
...
PHP;
    ...
    private function createViews(string $basePath, OutputInterface $output): void
    {
        ...
        foreach ($views as $filename => $content) {
            file_put_contents("{$dir}/{$filename}", $content);
            $output->writeln("  <comment>+</comment> resources/views/auth/{$filename}");
        }
    }
...
HTML;
    ...
    private function appendRoutes(string $basePath, OutputInterface $output): void
    {
...
PHP;
        ...
        $output->writeln("  <comment>+</comment> routes/web.php <info>(auth routes appended)</info>");
    }
}
